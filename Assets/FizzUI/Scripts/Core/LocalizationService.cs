...
namespace Fizz.UI.Core
{
    ...
    {
        private readonly string m_path = "FizzConfigration/";
        ...
        private void InitLangugageResources()
        {
            ...
            {
                JSONNode jsonClass = JSONClass.Parse(textAssets.text);

                m_resources = new Dictionary<string, string>();

                if (jsonClass != null && jsonClass.AsObject != null && jsonClass.AsObject.Count != 0)
                {
                    foreach (KeyValuePair<string, JSONNode> entry in jsonClass.AsObject)
                    {
